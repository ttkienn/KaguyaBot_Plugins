...
export default {
    ...
    cmd: async ({ kaguya, event, args, bot }) => {
        var autodown = bot.global.autodown;
        autodown[event.threadID] || (autodown[event.threadID] = "on"), autodown[event.threadID] && (autodown[event.threadID] = autodown[event.threadID] = "off", kaguya.reply(`Đã ${autodown[event.threadID] ? "bật" : "tắt"} chế độ tự động tải video hoặc nhạc cho nhóm này!\nThreadID: ${event.threadID}`));
    },
    events: async ({ kaguya, api, event, bot }) => {
