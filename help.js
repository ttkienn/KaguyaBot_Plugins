class HelpCommand {
  ...
  async cmd({ event, args, bot }, page = 1) {
    ...
    if (args[0]) {
      const plugin = bot.plugins.find(({ name }) => name === args[0]);
      if (plugin) {
  ...
  async reply({ event }) {
    if (!onReply || onReply.author !== event.senderID) {
      return reply("Bạn không phải người gửi tin nhắn này!");
    }
